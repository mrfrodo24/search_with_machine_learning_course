...
class DataPrepper:
    ...
    def __log_ltr_query_features(self, query_id, key, query_doc_ids, click_prior_query, no_results, terms_field="_id"):
        ...
        feature_results["sku"] = []
        # Feature names added to the dict down below
        if response and len(response['hits']) > 0 and len(response['hits']['hits']) > 0:
            for product in response['hits']['hits']:
                feature_results["doc_id"].append(product["_id"])
                feature_results["query_id"].append(query_id)
                feature_results["sku"].append(product["_id"])
                for feature in product["fields"]["_ltrlog"][0]["log_entry"]:
                    if not feature["name"] in feature_results:
                        feature_results[feature["name"]] = []
                    feature_results[feature["name"]].append(feature["value"] if "value" in feature else 0)
        else:
